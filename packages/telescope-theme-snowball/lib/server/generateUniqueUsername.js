Meteor.methods({
  generateUniqueUsername: function(name){
    ...
    _.each(diacriticsMap, function(diacriticMap) {
      name = name.replace(diacriticMap.letters, diacriticMap.base);
    });
    ...
    var user = Users.findOne({username: name});
    if(user){
      // console.log('exists, creating new postfix');
      var prefix = name + '-';
      var re = new RegExp('^' + prefix + '\\d+$');
      var postfix = 1;
      Users.find({username: re}).forEach(function(user){
        var userPostfix = parseInt(user.username.replace(prefix, ''), 10);
        if(userPostfix >= postfix){
          postfix = userPostfix + 1;
        }
      });
