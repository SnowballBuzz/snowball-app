Meteor.methods({
  bulkInvite: function (people, groupId, subject, html) {
    ...
    _.each(people, function (person) {
      // console.log(person, person.email, person.name)
      ...
      FlowRouter.path('Channel', {id: groupId});
      var link = '<a href="' + url + '">Sign in to join the discussion</a>';
      html = html.replace('{recipient_name}', person.name).replace('{link}', link);
      subject = subject.replace('{recipient_name', person.name);
      var content = Telescope.email.buildTemplate(html);
      // console.log(html, subject, content);
      Meteor.setTimeout(function () {
        Telescope.email.send(person.email, subject, content);
      }, 1);
    });
